...
//  cout<<endl;
    ...
    if((ha[n/2]-ha[0]*p[n/2])==(ha[n-1]-ha[n/2]*p[n/2])) { if(check(1,n/2)) {ans++;w=0;} }
//  cout<<ha[n/2-1]<<" "<<ha[n-2]-ha[n/2-1]*p[n/2]<<" "; 
    if(ha[n/2-1]==(ha[n-2]-ha[n/2-1]*p[n/2])) {if(check(0,n/2-1)) {ans++;w=n-1;}}
    if(ha[n/2-1]==(ha[n-1]-ha[n/2]*p[n/2]))  {if(check(0,n/2-1)) {ans++;w=n/2;}}
    for(int k=1;k<n/2;k++) 
    {
