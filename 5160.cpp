...
inline void read()
{
    c=getchar();
    while(c=='\n' || c=='\r') c=getchar();
    while(c!='\n' && c!='\r'){if(c!=' ') mp[0]+=c;c=getchar();}
    scanf("%lld",&n);
    for(i=1;i<=n;i++)
    {
        while(c=='\n' || c=='\r') c=getchar();
        while(c!='\n' && c!='\r'){if(c!=' ')mp[i]+=c;c=getchar();}
    }
}
            ...
            for(i=0;i<=len;i++)
            {
                ...
                else
                {
                    ...
                    while(1)
                    {
                        ...
                        ll num2=num.top();num.pop();
                        if(c=='*') num.push((num2*num1)%mod);
                        if(c=='+') num.push((num2+num1)%mod);//push 10
                        if(c=='-') num.push((num2-num1)%mod);//push -
                        if(c=='^') num.push(ksm(num2,num1));//push 9
                    }
                    ope.push(str[i]);
                }
            }
