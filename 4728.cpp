...
void add(int l,int r,int x){
	...
	for(int i=l;id[i]==sid;i++) a[i]+=x;
	SORT(sid);
	for(int i=sid+1;i<eid;i++) b[i]+=x;
	for(int i=r;id[i]==eid;i--) a[i]+=x;
	SORT(eid);
}
int query(int l,int r,int c){//<c^2
	int sid=id[l],eid=id[r],cnt=0;
	if(sid==eid){
		for(int i=l;i<=r;i++){
			if(a[i]+b[id[i]]<c*c) cnt++;
		}
		return cnt;
	}
